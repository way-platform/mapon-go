...
type ListUnitsRequest struct {
	...
	// Include is a list of additional data to include in the response.
		// Possible values: "fuel", "drivers", "location", "routes", "can", "weights", "ev_values", "altitude", "adblue".
		Include []string
	}
	...
	type jsonUnit struct {
		...
		Can *struct {
			Odom      interface{} `json:"odom"`
			FuelTotal interface{} `json:"fuel_total"`
		} `json:"can"`
	
		Weights *struct {
			CombinationWeight interface{} `json:"combination_weight"`
		} `json:"weights"`
	
		EvValues *struct {
			CanEvBatteryRel interface{} `json:"can_ev_battery_rel"`
			CanEvBatteryAbs interface{} `json:"can_ev_battery_abs"`
			EvCharging      interface{} `json:"ev_charging"`
		} `json:"ev_values"`
	...
	func mapJSONUnitToProto(j jsonUnit) *maponv1.Unit {
		...
		// CAN fields
		if j.Can != nil {
			if j.Can.FuelTotal != nil {
				v, _ := parseFloat(j.Can.FuelTotal)
				state.SetTotalFuelUsedLifetimeL(v)
			}
			// Assuming CAN odometer might be preferred if mileage is GPS?
			// Mapon doc says can.odom is CUMULATIVE ODOMETER_KM
			// However, u.SetOdometerM(int64(j.Mileage)) is already set from top level.
			// We could overwrite or check.
		}
	
		// Weights
		if j.Weights != nil {
			if j.Weights.CombinationWeight != nil {
				v, _ := parseFloat(j.Weights.CombinationWeight)
		...
		// EV Values
		if j.EvValues != nil {
			if j.EvValues.CanEvBatteryRel != nil {
				v, _ := parseFloat(j.EvValues.CanEvBatteryRel)
				state.SetBatterySocPercent(v)
			}
			if j.EvValues.CanEvBatteryAbs != nil {
				v, _ := parseFloat(j.EvValues.CanEvBatteryAbs)
				state.SetBatterySocKwh(v)
			}
			if j.EvValues.EvCharging != nil {
				v, _ := parseFloat(j.EvValues.EvCharging)
