...
func (c *Client) ListCanPeriodData(ctx context.Context, request *ListCanPeriodDataRequest, opts ...ClientOption) (_ *ListCanPeriodDataResponse, err error) {
	...
	for _, u := range responseBody.Data.Units {
		...
		ucpd.SetFuelLevel(mapCanMetricList(u.FuelLevel))
		ucpd.SetServiceDistance(mapCanMetricList(u.ServiceDistance))
		ucpd.SetTotalDistance(mapCanMetricList(u.TotalDistance))
		ucpd.SetTotalFuel(mapCanMetricList(u.TotalFuel))
		ucpd.SetTotalEngineHours(mapCanMetricList(u.TotalEngineHours))
		ucpd.SetAmbientTemp(mapCanMetricList(u.AmbientTemp))
