...
type File struct {
	...
	Region mapon.Region `json:"region"`
...
func NewOAuth2Client(ctx context.Context, opts ...mapon.ClientOption) (*mapon.Client, error) {
	cf, err := ReadFile()
	if err != nil {
		return nil, err
	}
	if cf.Token == nil || cf.Token.Expiry.Before(time.Now()) {
		return nil, fmt.Errorf("invalid token, please login using `mapon auth login`")
	}
	return mapon.NewClient(
		ctx,
		append(
			opts,
			mapon.WithRegion(cf.Region),
			mapon.WithOAuth2TokenSource(oauth2.StaticTokenSource(cf.Token)),
		)...,
	)
}

// NewClientWithAPIKey creates a new Mercedes-Benz API client using the API key from the CLI credentials.
func NewClientWithAPIKey(ctx context.Context, opts ...mapon.ClientOption) (*mapon.Client, error) {
	cf, err := ReadFile()
	if err != nil {
		return nil, err
	}
