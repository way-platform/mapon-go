...
func newLoginCommand() *cobra.Command {
	...
	region := cmd.Flags().String("region", string(mapon.RegionECE), "region to use for authentication (OAuth2 only)")
	...
	cmd.RunE = func(cmd *cobra.Command, _ []string) error {
		...
		authFile := File{
			Region: mapon.Region(*region),
			APIKey: *apiKey,
		}
		if *clientSecret != "-" && *clientID != "-" {
			config, err := mapon.NewOAuth2Config(mapon.Region(*region), *clientID, *clientSecret)
			if err != nil {
				return err
			}
