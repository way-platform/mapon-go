...
func ChunkAPI() error {
	mg.Deps(DownloadAPI)
	if err := os.RemoveAll(root("docs/chunks")); err != nil {
		return err
	}
	return cmd(root("tools"), "go", "run", "./cmd/chunk-html-docs", "-input", root("docs/api.html"), "-output", root("docs/chunks")).Run()
}
